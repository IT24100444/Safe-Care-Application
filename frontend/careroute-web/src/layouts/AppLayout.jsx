...
export default function AppLayout() {
  ...
  return <div className="site-shell"><header className="site-header"><div className="header-inner"><Link className="brand" to="/">{t('appName')}</Link><nav aria-label="Main"><Link to="/facilities">Facilities</Link><Link to="/services">Services</Link><Link to="/doctors">Doctors</Link>{user?.role === 'DOCTOR'&&<><Link to="/doctor/profile">My Doctor Profile</Link><Link to="/doctor/schedules">My Schedule</Link></>}{user?.role === 'ADMIN' && <><Link to="/admin/doctors">Manage doctors</Link><Link to="/admin/schedules">Manage schedules</Link><Link to="/admin/facilities">Manage facilities</Link><Link to="/admin/services">Manage services</Link></>}{isAuthenticated ? <Link to="/profile">{t('auth.profile')}</Link> : <><Link to="/login">{t('auth.login')}</Link><Link to="/register">{t('auth.register')}</Link></>}</nav><LanguageSelector /></div></header><main className="page-content" id="main-content"><Outlet /></main><footer className="site-footer">{t('footer')}</footer></div>
}
